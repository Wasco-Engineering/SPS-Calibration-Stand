...
| Item | Value |
|------|--------|
| Host / branch habit | `ID-MAN-SPS-01` |
| Stand / `equipment_id` | **`ID-SPS-01`** (detail rows: `ID-SPS-01-L` / `ID-SPS-01-R`) |
| Env (preferred) | `STINGER_STAND_ID=ID-SPS-01`, `STINGER_CONFIG_DIR=C:\Stinger` |
| Config dir | `C:\Stinger` |
| Typical peers | Idaho `ID-SPS-02` / legacy `STINGER_04`; CA `CA-SPS-*` |

Legacy labels `STINGER_03` may still appear in old DB rows or env on this PC — new writes
must use **`ID-SPS-01`**.
...
- `test_parameters.equipment_id` — must stay **`ID-SPS-01`** here
- `hardware.labjack.local_barometric_psi` — site gauge zero (Idaho ~13.49 here)
...
```text
equipment_id, local_barometric_psi, baudrate, open_fitting, COM ports
...
3. Prefer merging `origin/main` into the stand branch (`ID-MAN-SPS-01`), not resetting

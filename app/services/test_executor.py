...
class CyclePhaseRunner:
    ...
    def _run_nc_derived_vacuum_cycle(
        ...
        *,
        ...
        """Cycle a one-input vacuum switch via low reset then upward activation.

        Reposition below the band (deactivation edge on the descending leg when
        travel crosses the switch window), then sweep upward for activation.
        ...
        deactivation_detected, deactivation_diagnostic = self._ctx._wait_for_cycle_edge(
            ...
            allow_no_switch_at_target=True,
        )
        if self._ctx._cancel_event.is_set():
            return
        if not deactivation_detected and deactivation_diagnostic != 'NO_SWITCH_DETECTED':
            self._fail_missing_cycle_edge('deactivation', deactivation_diagnostic)
        if not deactivation_detected:
            logger.info(
                '%s: NC-derived cycle already on low reset side without deactivation transition; '
                'continuing upward for activation',
                self._ctx._port_id,
            )

        reading = self._ctx._get_latest_reading(self._ctx._port_id)
        if reading is not None and reading.switch is not None:
            self._ctx._cycle_debounce_state = self._ctx._spdt_debounce_from_switch(reading.switch)
...
class PrecisionPhaseRunner:
    ...
    def run_precision_sweep(
        ...
        if self._ctx._uses_nc_derived_vacuum_window():
            logger.info(
                '%s: Precision NC-derived upward sweep reset=%.4f high=%.4f rate=%.4f psi/s',
                self._ctx._port_id,
                approach_target,
                target_out,
                self._ctx._slow_edge_rate_psi,
            )
            outcome = self._ctx._run_nc_derived_upward_precision_pass(
                reset_target=approach_target,
        ...
        if self._can_start_precision_from_current_reset_side(
            ...
            logger.info(
                '%s: Precision near approach (%.4f PSI) — settling at slow edge rate before sweep',
                self._ctx._port_id,
                approach_target,
            )
    ...
    def _can_start_precision_from_current_reset_side(
        ...
        if switch_state == self._ctx._target_switch_state_for_edge('activation'):
            return False
        # Must already be near the close-limit approach. Being merely on the
        # "reset" side of approach (e.g. mid-band below a decreasing approach)
        # used to skip the fast approach and start the slow out-sweep with too
        # little travel to capture the activation edge.
        # Use a tighter band than the general approach tolerance so we still
        # creep to the true approach when cycling left us a bit short/long.
        tolerance = min(
            max(0.02, self._ctx._precision_approach_tolerance_psi),
            0.06,
        )
        ready = abs(pressure - approach_target) <= tolerance
        if ready:
            logger.info(
        ...
        if not arrived:
            ...
            if (
                ...
                logger.warning(
                    ...
                    '(pressure=%.4f PSI), but switch is reset; continuing to settle',
                    self._ctx._port_id,
                    approach_target,
                    pressure,
                )
                ...
                return
        # Always downshift and dwell so the slow out-sweep does not inherit
        # residual fast-ramp momentum from cycling / approach.
        self._settle_at_precision_approach(approach_target)
...
class TestExecutor:
    ...
    def _wait_for_cycle_edge(
        ...
        allow_no_switch_at_target: bool = False,
            ...
            if pressure_test is not None:
                ...
                if (
                    ...
                    if wait_at_target > self._cycle_no_switch_grace_s:
                        if self._try_accept_port_cycle_edge(
                            edge_type, port_edges_before, samples_list
                        ):
                            self._hold_after_cycle_edge(
                                edge_type, samples_list[-1] if samples_list else None
                            )
                            self._cycle_waiting_edge = None
                            return True, None
                        if allow_no_switch_at_target:
                            logger.info(
                                '%s: No %s edge while repositioning to %.4f PSI; continuing',
                                self._port_id,
                                edge_type,
                                target_psi,
                            )
    ...
    def _prepare_switch_for_cycle_edge(
        ...
        *,
        ...
        # UC8A edge case: vacuum decreasing-activation, switch already at activated
        # state below the band — nudge up to reset before the activation sweep.
        if (
            edge_type == 'activation'
            and self._resolve_sweep_mode() == 'vacuum'
            and direction < 0
            ...
            logger.info(
                ...
                'nudging up to %.4f PSI to reset before activation leg',
                self._port_id,
                pressure,
                nudge_target,
            )
            self._emit_substate('cycling.prep_deactivated')
            self._set_active_test_route(sweep_mode, 'cycle prep reset')
            if not self._port.alicat.set_ramp_rate(self._fast_rate_psi):
                self._fail(
                    TestFailureCode.RAMP_RATE_FAILURE,
            ...
            prep_start = time.perf_counter()
            near_target_since: Optional[float] = None
            prep_state_reset = not self._cycle_target_switch_state(edge_type)
            while time.perf_counter() - prep_start < prep_timeout_s:
                if self._cancel_event.is_set():
                    return
                pressure_now, state = self._read_pressure_and_switch_state()
                if state == prep_state_reset:
                    reading = self._get_latest_reading(self._port_id)
                    if reading is not None and reading.switch is not None:
                        self._cycle_debounce_state = self._spdt_debounce_from_switch(reading.switch)
                    else:
                        self._cycle_debounce_state = SpdtDebounceState()
                    logger.info(
                        '%s: Cycle reset prep complete — switch now %s',
                        self._port_id,
    ...
    def _run_nc_derived_upward_precision_pass(
        self,
        reset_target: float,
        high_target: float,
        rate_psi_per_sec: float,
        ...
        if low_edge is None:
            ...
            if len(edges) < 1:
                logger.warning(
                    '%s: NC-derived precision first edge not detected target=%.3f',
                    self._port_id,
                    high_target,
                )
                return SweepPassOutcome(result=None, missing_edge='first')
            if len(edges) < 2:
                logger.warning(
                    '%s: NC-derived precision second edge not detected target=%.3f',
                    self._port_id,
                    high_target,
                )
                return SweepPassOutcome(result=None, missing_edge='second')
            ...
            if high_edge is None:
                logger.warning(
                    '%s: NC-derived precision upper edge not detected target=%.3f',
                    self._port_id,
                    high_target,
                )
                return SweepPassOutcome(result=None, missing_edge='second')
            low_pressure = low_edge.pressure_psi
            high_pressure = high_edge.pressure_psi

        direction = self._resolve_activation_sweep_direction()
        if direction > 0:
            if self._on_edge_detected:
                self._on_edge_detected('deactivation', low_pressure)
                self._on_edge_detected('activation', high_pressure)
            result = SweepResult(activation_psi=high_pressure, deactivation_psi=low_pressure)
        else:
            if self._on_edge_detected:
                self._on_edge_detected('activation', low_pressure)
                self._on_edge_detected('deactivation', high_pressure)
            result = SweepResult(activation_psi=low_pressure, deactivation_psi=high_pressure)

        return SweepPassOutcome(result=result, missing_edge=None)

    def _collect_edges_during_sweep(
        self,
        target_psi: float,
        direction: int,
        max_edges: int,
    ) -> list[EdgeDetection]:
        """Collect up to ``max_edges`` stable transitions while sweeping toward target."""
        if max_edges < 1:
            return []
        ...
        while time.perf_counter() - start < dynamic_timeout_s and len(edges) < max_edges:
            if self._cancel_event.is_set():
                return edges
    ...
    def _execute_out_back_sweep(
        ...
        if not self._precision_edges_ordered(direction, edge_out.pressure_psi, edge_back.pressure_psi):
            logger.warning(
                '%s: Rejecting inverted precision edges act=%.4f deact=%.4f '
                '(direction=%s) — likely turnaround bounce',
                self._port_id,
                edge_out.pressure_psi,
                edge_back.pressure_psi,
                'increasing' if direction > 0 else 'decreasing',
            )
            return SweepPassOutcome(result=None, missing_edge='second')
    ...
    def _precision_return_edge_gate_psi(self, activation_psi: float) -> float:
        """Minimum travel past activation before a return-edge is trusted."""
        del activation_psi
        act_est, deact_est = self._ordered_cycle_estimates()
        if act_est is not None and deact_est is not None:
            hysteresis = abs(deact_est - act_est)
            if hysteresis > 1e-4:
                return max(0.03, min(0.15, 0.35 * hysteresis))
        return max(0.05, 0.25 * self._precision_close_limit_offset_psi)

    @staticmethod
    def _precision_edges_ordered(
        direction: int,
        ...
        eps = 1e-4
        if direction < 0:
            return deactivation_psi + eps >= activation_psi
        return activation_psi + eps >= deactivation_psi
    ...
    def _sweep_to_edge(
        ...
        *,
        ...
        Args:
            ...
            require_past_psi: When set, ignore edges until pressure has crossed
                this value in the sweep direction by ``past_margin_psi``.
        ...
        if reading_start is not None and reading_start.switch is not None:
            debounce_state = self._spdt_debounce_from_switch(reading_start.switch)
        if seed_edge_time:
            debounce_state = replace(debounce_state, last_edge_time=time.perf_counter())
        initial_switch_state: Optional[bool] = debounce_state.committed_activated

        def _past_gate(pressure_test: float) -> bool:
            if require_past_psi is None or not math.isfinite(pressure_test):
                return True
            margin = max(0.0, past_margin_psi)
            if direction > 0:
                return pressure_test >= require_past_psi + margin
            return pressure_test <= require_past_psi - margin
            ...
            if self._cycle_edge_switch_state_allowed(edge_type, initial_switch_state):
                ...
                if start_abs is not None:
                    ...
                    if math.isfinite(edge_pressure):
                        ...
                        elif not _past_gate(edge_pressure):
                            logger.debug(
                                '%s: Initial %s switch state matches at %.4f PSI, '
                                'but has not cleared gate past %.4f (margin=%.4f); waiting',
                                self._port_id,
                                edge_type,
                                edge_pressure,
        ...
        start = time.perf_counter()
        target_reached_since: Optional[float] = None
        level_detect_since: Optional[float] = None
        settle_window_s = max(0.08, self._stable_count * 0.02)
        target_grace_s = settle_window_s + self._precision_post_target_grace_s
        ...
        while time.perf_counter() - start < dynamic_timeout_s:
            ...
            if committed_state is not None and math.isfinite(pressure):
                ...
                edge_pressure = committed_pressure if committed_pressure is not None else pressure
                if not _past_gate(edge_pressure):
                    logger.debug(
                        '%s: Ignoring %s edge at %.4f PSI before clearing gate past %.4f '
                        '(margin=%.4f)',
                        self._port_id,
                        edge_type or 'edge',
                        edge_pressure,
            ...
            # Return-sweep turnaround bounce can leave the switch already in the
            # deactivation state. Once past the activation gate and stable for a
            # short dwell, accept that level instead of waiting forever for a
            # transition that will not occur.
            if (
                require_past_psi is not None
                and edge_type == 'deactivation'
                and _past_gate(pressure)
                and self._cycle_edge_switch_state_allowed(edge_type, current_state)
            ):
                now = time.perf_counter()
                if level_detect_since is None:
                    level_detect_since = now
                elif now - level_detect_since >= max(0.12, self._precision_approach_settle_s * 0.5):
                    logger.info(
                        '%s: Deactivation level-detect at %.4f PSI after clearing activation gate',
                        self._port_id,
                        pressure,
                    )
                    if self._on_edge_detected:
                        self._on_edge_detected(edge_type, pressure)
                    return EdgeDetection(pressure_psi=pressure, activated=current_state)
            else:
                level_detect_since = None
    ...
    def _uses_single_sense_switch_derivation(self) -> bool:
        """True when NO/NC are derived from one sensed line (PTP or port config)."""
        ...
        if _config_bool(port_cfg.get('switch_nc_derived_from_no')):
            return True
        return _config_bool(port_cfg.get('switch_no_derived_from_nc'))

    def _accept_observed_switch_polarity_for_cycle_edges(self) -> bool:
        """When True, the first transition on each leg defines activation polarity."""
        if (
    ...
    def _cycle_edge_switch_state_allowed(self, edge_type: str, switch_activated: bool) -> bool:
        ...
        if (
            edge_type in {'activation', 'deactivation'}
            and self._accept_observed_switch_polarity_for_cycle_edges()
        ):
            return True
    ...
    def _cycle_ramp_past_target_plausible(
        self,
        edge_type: str,
        pressure_test: float,
        target_test: float,
    ) -> bool:
        """Ignore bogus low readings that would mark a descending ramp as already at target."""
        if not math.isfinite(pressure_test) or not math.isfinite(target_test):
            return False
        if self._resolve_sweep_mode() != 'vacuum' or edge_type != 'activation':
            return True
        min_psi, max_psi = self._resolve_sweep_bounds()
        if pressure_test < min_psi - 2.0:
            return False
        if pressure_test > max_psi + 1.0 and target_test < min_psi:
            return False
        return True
    ...
    def _ordered_cycle_estimates(self) -> tuple[Optional[float], Optional[float]]:
        ...
        the correct semantics. Near-zero hysteresis (equal means) gets a
        tiny synthetic separation so precision bracketing still validates.
        """
        ...
        if activation is not None and deactivation is not None:
            ...
            # Equal/near-equal means reject cycle-estimate targets and fall back
            # to a full PTP span; nudge so act/deact keep semantic order.
            min_sep = 1e-3
            if abs(activation - deactivation) < min_sep:
                nudge = max(
                    min_sep,
                    getattr(self, '_precision_close_limit_offset_psi', 0.05) * 0.05,
                )
                mid = 0.5 * (activation + deactivation)
                if direction < 0:
                    activation, deactivation = mid - 0.5 * nudge, mid + 0.5 * nudge
                else:
                    deactivation, activation = mid - 0.5 * nudge, mid + 0.5 * nudge
                logger.info(
                    '%s: Near-equal cycle estimates; applying %.4f PSI separation '
                    '(act=%.4f deact=%.4f)',
                    self._port_id,
                    nudge,
                    activation,
    ...
    def _transducer_installed(self) -> bool:
        port_cfg = self._config.get('hardware', {}).get('labjack', {}).get(self._port_id, {})
